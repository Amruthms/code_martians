...
export function LiveMonitoring() {
  ...
  const [selectedPPE, setSelectedPPE] = useState<string[]>(['helmet', 'vest', 'harness']);
  ...
  return (
    <div className="p-6 space-y-6 bg-[#0A0A0A]">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400">Real-time AI-powered PPE detection across all camera feeds</p>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="mt-1 text-2xl text-white">{cameraFeeds.length}</div>
              </div>
          ...
          <div>
            ...
            <Tabs value={selectedZone} onValueChange={setSelectedZone}>
              <TabsList className="bg-gray-800">
                <TabsTrigger value="all" className="data-[state=active]:bg-[#FF7A00] data-[state=active]:text-white">All Zones</TabsTrigger>
                <TabsTrigger value="Level 1" className="data-[state=active]:bg-[#FF7A00] data-[state=active]:text-white">Level 1</TabsTrigger>
                <TabsTrigger value="Level 2" className="data-[state=active]:bg-[#FF7A00] data-[state=active]:text-white">Level 2</TabsTrigger>
                <TabsTrigger value="Roof" className="data-[state=active]:bg-[#FF7A00] data-[state=active]:text-white">Roof</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>

          <div>
            <div className="text-sm mb-2 text-gray-400">PPE Detection Toggle</div>
            <div className="flex gap-2">
              <Button
                ...
                className={selectedPPE.includes('helmet') ? 'bg-[#FF7A00] text-white hover:bg-[#FF7A00]/90' : 'border-gray-700 text-gray-300 hover:bg-gray-800'}
              >
              ...
              <Button
                ...
                className={selectedPPE.includes('vest') ? 'bg-[#FF7A00] text-white hover:bg-[#FF7A00]/90' : 'border-gray-700 text-gray-300 hover:bg-gray-800'}
              >
                <Shirt className="h-4 w-4 mr-2" />
                Vest
              </Button>
              <Button
                variant={selectedPPE.includes('harness') ? 'default' : 'outline'}
                size="sm"
                onClick={() => {
                  setSelectedPPE(prev => 
                ...
                className={selectedPPE.includes('harness') ? 'bg-[#FF7A00] text-white hover:bg-[#FF7A00]/90' : 'border-gray-700 text-gray-300 hover:bg-gray-800'}
              >
                Harness
              </Button>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredFeeds.filter(f => f.id !== 9).map((feed) => (
          <Card key={feed.id} className="overflow-hidden bg-gray-900 border-gray-800">
              ...
              <div className="aspect-video bg-black flex items-center justify-center relative">
                ...
                {feed.workers > 0 && (
                  <div className="absolute inset-0 p-4">
                    {Array.from({ length: Math.min(feed.workers, 3) }).map((_, i) => (
                      <div
                        key={i}
                        className={`
                          absolute border-2 rounded
                          ${feed.alerts.length > 0 && i === 0 ? 'border-red-500' : 'border-green-500'}
                        `}
                        style={{
                          left: `${20 + i * 25}%`,
                          top: `${30 + i * 10}%`,
            ...
            <CardContent className="p-4 bg-gray-900">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-1">{feed.workers} workers detected</div>
                </div>
              ...
              {feed.alerts.length > 0 && (
                <div className="space-y-2 mt-3 pt-3 border-t border-gray-800">
                  ...
                  <div className="flex gap-2 mt-2">
                    <Button size="sm" variant="outline" className="flex-1 text-xs h-7 border-gray-700 text-gray-300 hover:bg-gray-800">
                    ...
                    <Button 
                      ...
                      onClick={() => updateCameraFeed(feed.id, { alerts: [], status: 'safe' })}
                    >
